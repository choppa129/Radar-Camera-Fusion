...
class ZCAN(object):
    def __init__(self):
        if platform.system() == "Windows":
            # 驱动库位于 driver\，其内核库 kerneldlls\ 与 dll 同级，
            # 需要把该目录加入 DLL 搜索路径才能被 dll 内部加载到。
            if hasattr(os, "add_dll_directory"):
                self.__dll_dir = os.add_dll_directory(paths.DRIVER_DIR)
            self.__dll = windll.LoadLibrary(paths.ZLG_DLL_PATH)
        else:
